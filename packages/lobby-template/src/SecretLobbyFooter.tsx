...
// SecretLobbyFooter
// -----------------------------------------------------------------------------
// "Powered by SecretLobby" attribution footer rendered on every published
// lobby and login page (plus the editor preview, so designers see what users
// will see). Pure in-flow markup — sits at the bottom of whatever container
// the caller drops it into, no fixed/sticky positioning.
//
// The logo is inlined as an SVG so we don't ship a separate asset request —
// keeps the footer self-contained and zero-RTT (matters on the first paint
// of a public lobby where every byte counts). Source: the brand mark in
// `apps/marketing/public/secret-lobby-logo.svg` — colors are inlined here
// directly so the SVG renders identically whether the lobby's theme is
// light or dark.
//
// Both anchors include `cursor-pointer` per the project-wide UI rule (see
// CLAUDE.md — every `<a>` and `<button>` must have it).
// =============================================================================

export interface SecretLobbyFooterProps {
  /** Optional extra classes for layout overrides (e.g. more vertical
   *  breathing room in a caller that needs it). */
  className?: string;
  /** When true, the footer is pinned to the viewport bottom with
   *  `position: fixed` (plus a subtle translucent backdrop so it stays
   *  readable over any lobby palette). Callers that opt in must add
   *  enough bottom padding to their page wrapper to clear it — the
...
export function SecretLobbyFooter({
  ...
  return (
    <footer
      className={`w-full py-4 px-4 text-center text-xs flex items-center justify-center gap-1.5 ${positionClasses} ${className ?? ""}`.trim()}
      style={{
    ...
    >
      <a
        href="https://secretlobby.co"
        target="_blank"
        rel="noopener noreferrer"
        aria-label="SecretLobby"
      ...
      <span>
        ...
        {" · "}
        <a
          href="https://secretlobby.co/terms"
          target="_blank"
          rel="noopener noreferrer"
          className="underline cursor-pointer"
        >
...
// Inlined brand mark — the red-circle "SECRET / LOBBY" logo. Rendered at a
// small fixed pixel size so the footer doesn't scale-bloat on retina; the
// SVG paints crisp at any DPR. Colors are hardcoded to the brand hexes
// (`#ed1b2f` red, `#231f20` near-black) on purpose — the mark is a brand
// asset, not a themable element.
function SecretLobbyMark() {
  return (
    <svg
