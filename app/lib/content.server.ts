...
export interface SiteContent {
  ...
  sitePassword?: string;
  theme?: ThemeSettings;
  allowUserColorMode?: boolean; // If false, forces light mode and hides toggle
}
...
export async function getThemeSettings(): Promise<ThemeSettings> {
  ...
  // Ensure colorMode exists for backwards compatibility
  if (!theme.colorMode) {
...
export function generateThemeCSS(theme: ThemeSettings): string {
  ...
  // Return CSS without newlines for safe embedding in inline scripts
  return [
...
export async function getAllowUserColorMode(): Promise<boolean> {
  const content = await getSiteContent();
  // Default to true (allow user to toggle)
