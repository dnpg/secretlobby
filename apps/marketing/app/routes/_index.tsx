...
function LogoDistortionBackground() {
  ...
  // Ripple trail - stores recent mouse positions for trail effect
  ...
  useEffect(() => {
    ...
    // Logo physics state - positions stored in screen-normalized coords (0-1)
    interface Logo {
      x: number; y: number;  // 0-1 normalized, Y=0 at top, Y=1 at bottom (screen coords)
      vx: number; vy: number;
      size: number;
      rotation: number;
      angularVel: number;
    ...
    for (let i = 0; i < NUM_LOGOS; i++) {
      ...
      logos.push({
        x: Math.random(),
        y: Math.random(),  // Y=0 top, Y=1 bottom (screen coords)
        vx: 0,
        vy: 0,
        size,
    ...
    logos.sort((a, b) => a.depth - b.depth);

    // Offscreen canvas for logos
    const logoCanvas = document.createElement("canvas");
    const logoCtx = logoCanvas.getContext("2d")!;
    let logoTexture: WebGLTexture | null = null;
    let needsLogoRedraw = true;

    // Load logo image
    const logoImg = new Image();
    logoImg.src = "/secret-lobby-logo.svg";

    // Mouse tracking - screen coords (Y=0 top, Y=1 bottom)
    const handleMouseMove = (e: MouseEvent) => {
      const rect = canvas.getBoundingClientRect();
      const newX = (e.clientX - rect.left) / rect.width;
      const newY = (e.clientY - rect.top) / rect.height;  // Y=0 at top, Y=1 at bottom
      ...
      // Add to ripple trail if moving fast enough
      if (speed > 0.002) {
        ...
        if (rippleTrailRef.current.length > 20) {
          rippleTrailRef.current.shift();
        }
      }
    ...
    // Water shader with realistic ripple distortion near mouse
    const vertexShader = `
      attribute vec2 a_position;
      varying vec2 v_uv;
      void main() {
    ...
    // Max trail points to pass to shader
    const MAX_TRAIL_POINTS = 12;

    const fragmentShader = `
      precision highp float;
      uniform sampler2D u_logos;
      uniform float u_time;
      uniform vec2 u_mouse;
      uniform float u_rippleStrength;
      uniform vec2 u_resolution;
      uniform vec4 u_trail[${MAX_TRAIL_POINTS}];  // xy = position, z = strength, w = age
      ...
      // Calculate ripple effect for a single point
      vec3 calcRipple(vec2 uv, vec2 ripplePos, float strength, float age, float aspect, float time) {
        ...
        float expandRadius = age * 0.3;  // How far the ripple has traveled
        float ringWidth = 0.04;

        // Ring that expands outward
        float ring = smoothstep(ringWidth, 0.0, abs(dist - expandRadius));

        // Also keep center ripple for recent touches
        float centerRipple = smoothstep(0.1, 0.0, dist) * max(0.0, 1.0 - age * 2.0);

        float intensity = (ring + centerRipple) * strength * max(0.0, 1.0 - age * 0.8);

        // Wave physics
        float waveSpeed = 4.0;
        float waveFreq = 40.0;
        float phase = dist * waveFreq - time * waveSpeed - age * 10.0;
        float wave = sin(phase) * exp(-dist * 4.0) * intensity;

        // Slope for refraction
      ...
      void main() {
        ...
        // Process all trail ripples
        for (int i = 0; i < ${MAX_TRAIL_POINTS}; i++) {
          if (i >= u_trailCount) break;
          vec4 trail = u_trail[i];
          if (trail.z > 0.01) {  // Only if strength > threshold
            vec3 ripple = calcRipple(uv, trail.xy, trail.z, trail.w, aspect, u_time);
            totalRefract += ripple.xy;
            totalWaveHeight += ripple.z;
            totalIntensity += trail.z * max(0.0, 1.0 - trail.w);
          }
        }

        // Current mouse position ripple (strongest)
        if (u_rippleStrength > 0.01) {
          ...
          float inZone = smoothstep(0.12, 0.0, dist);
          inZone *= inZone;
          ...
          totalWaveHeight += (wave + wave2) * inZone * u_rippleStrength;
        ...
        vec4 logoColor = texture2D(u_logos, uv + totalRefract);

        // Chromatic aberration
        float aberr = abs(totalWaveHeight) * 0.004;
        vec4 cR = texture2D(u_logos, uv + totalRefract + vec2(aberr, 0.0));
        vec4 cB = texture2D(u_logos, uv + totalRefract - vec2(aberr, 0.0));
        logoColor.r = mix(logoColor.r, cR.r, min(1.0, totalIntensity) * 0.4);
        logoColor.b = mix(logoColor.b, cB.b, min(1.0, totalIntensity) * 0.4);

        // Dark water
        vec3 water = vec3(0.006, 0.006, 0.01);
        vec3 color = mix(water, logoColor.rgb, logoColor.a);

        // White highlights on wave crests
        float highlight = max(0.0, totalWaveHeight) * 0.25;
        color += vec3(0.95, 0.97, 1.0) * highlight;
    ...
    // Compile shader
    function compileShader(glCtx: WebGLRenderingContext, src: string, type: number) {
    ...
    const rippleStrengthLoc = gl.getUniformLocation(program, "u_rippleStrength");
    const resLoc = gl.getUniformLocation(program, "u_resolution");
    const logoTexLoc = gl.getUniformLocation(program, "u_logos");
    const trailLoc = gl.getUniformLocation(program, "u_trail");
    const trailCountLoc = gl.getUniformLocation(program, "u_trailCount");

    // Fix texture orientation - flip Y so canvas top = texture top
    ...
    // Physics constants - gentle push
    const PUSH_FORCE = 0.0008;
    const DRAG = 0.92; // Water drag
    ...
    function resize() {
      ...
      if (canvas.width !== w || canvas.height !== h) {
        ...
        logoCanvas.width = w;
        logoCanvas.height = h;
        needsLogoRedraw = true;
      }
    }

    function drawLogos() {
      if (!logoCtx || !logoImg.complete) return;
      ...
      for (const logo of logos) {
        // Logo coords: x,y are 0-1 screen coords (Y=0 top, Y=1 bottom)
        const x = logo.x * w;
        const y = logo.y * h;
        const size = logo.size * Math.max(w, h);
        const alpha = 0.15 + logo.depth * 0.35;

        logoCtx.save();
        logoCtx.translate(x, y);
        logoCtx.rotate(logo.rotation);
        logoCtx.globalAlpha = alpha;

        logoCtx.drawImage(logoImg, -size / 2, -size / 2, size, size);

        // Red tint
        logoCtx.globalCompositeOperation = "source-atop";
        logoCtx.fillStyle = `rgba(237, 27, 47, ${0.45 - logo.depth * 0.35})`;
        logoCtx.fillRect(-size / 2, -size / 2, size, size);
    ...
    function updatePhysics(aspect: number) {
      ...
      // Aspect-corrected push radius (in normalized coords)
      const pushRadiusBase = 0.08;

      for (const logo of logos) {
        // Calculate distance with aspect ratio correction
        const dx = (logo.x - mx) * aspect;  // Scale X by aspect
        const dy = logo.y - my;
        const dist = Math.sqrt(dx * dx + dy * dy);

        // Push radius also needs aspect correction
        const effectiveRadius = pushRadiusBase * aspect;

        // Gentle push force when mouse is near
        if (dist < effectiveRadius && dist > 0.001) {
          const strength = (1 - dist / effectiveRadius);
          const force = strength * strength * PUSH_FORCE / logo.mass;

          // Push direction (normalize the aspect-corrected vector)
          const pushDirX = dx / dist;
          const pushDirY = dy / dist;

          // Apply push (convert X back from aspect-corrected)
          logo.vx += (pushDirX / aspect) * force;
          logo.vy += pushDirY * force;

          // Spin when pushed
          logo.angularVel += (Math.random() - 0.5) * force * 40;
        ...
        // Angular drag
        if (Math.abs(logo.angularVel) > 0.0001) {
          logo.angularVel *= ANGULAR_DRAG;
          logo.rotation += logo.angularVel;
          anyMoved = true;
        } else {
          logo.angularVel = 0;
        }
    ...
    const startTime = performance.now();
    let rippleIntensity = 0;  // Fades when mouse stops
    ...
    function render() {
      ...
      if (needsLogoRedraw && logoImg.complete) {
        drawLogos();
      ...
      // Ripple intensity - active when moving, fades when stopped
      const timeSinceMove = now - mouseRef.current.lastMoveTime;
      if (timeSinceMove < 50) {
        // Mouse moving - increase intensity
        rippleIntensity = Math.min(1.0, rippleIntensity + 0.15);
      } else {
        // Mouse stopped - fade out
        rippleIntensity *= 0.95;
      }

      // Clean up old trail points and prepare trail data for shader
      const trailData = new Float32Array(MAX_TRAIL_POINTS * 4);
      let trailCount = 0;
      const maxAge = 1.5;  // seconds before trail point disappears

      // Filter and update trail points
      rippleTrailRef.current = rippleTrailRef.current.filter(point => {
        const age = (now - point.time) / 1000;
        return age < maxAge;
      });

      // Pack trail data for shader (most recent first for priority)
      const recentTrail = rippleTrailRef.current.slice(-MAX_TRAIL_POINTS).reverse();
      for (let i = 0; i < recentTrail.length && i < MAX_TRAIL_POINTS; i++) {
        const point = recentTrail[i];
        const age = (now - point.time) / 1000;
        const idx = i * 4;
        trailData[idx] = point.x;  // x position
        trailData[idx + 1] = 1.0 - point.y;  // y position (flip for shader coords)
        trailData[idx + 2] = point.strength * (1.0 - age / maxAge);  // fading strength
        trailData[idx + 3] = age;  // age in seconds
        trailCount++;
      }
      ...
      // Convert mouse Y to shader coords (flip Y: shader Y=0 bottom, Y=1 top)
      gl.uniform2f(mouseLoc, mouseRef.current.x, 1.0 - mouseRef.current.y);
      gl.uniform1f(rippleStrengthLoc, rippleIntensity);
      gl.uniform2f(resLoc, canvas.width, canvas.height);

      // Pass trail data
      gl.uniform4fv(trailLoc, trailData);
      gl.uniform1i(trailCountLoc, trailCount);

      gl.activeTexture(gl.TEXTURE0);
      gl.bindTexture(gl.TEXTURE_2D, logoTexture);
      gl.uniform1i(logoTexLoc, 0);
